...
* Forbidding creation of IngressController with CLB type on day-2, as well as changing the type to CLB.

## Proposal

The implementation will automatically configure publishing service for IngressControllers
...
1. The cluster administrator installs an OpenShift cluster on AWS with
   the `AWSDualStackInstall` featuregate enabled and [a dual-stack IP family](https://github.com/openshift/installer/pull/9930),
...
4. The ingress-operator does not create Alias AAAA record for the wildcard domain.

4. AWS provisions a standard IPv4-only NLB.

5. IngressControllers function normally with IPv4 connectivity only.
...
**Notes:**
- Installer provides an [install config validation for IP family and load balancer type](https://github.com/openshift/installer/pull/10256).
  It's impossible to request a creation of cluster with dualstack IP family and CLB type.
...
`AWSDualStackInstall` featuregate. Document the limitations and the
...
itself does not require a separate featuregate, as it automatically
...
- The ingress operator changes can be implemented and merged independently
  of `AWSDualStackInstall` installer feature gate state.
...
the installer's `AWSDualStackInstall` featuregate graduation.
...
- On clusters installed with `AWSDualStackInstall` featuregate and dual-stack IP family:
  ...
  - IngressControllers publishing service will be reconciled and the `ipFamilies` and
    `ipFamilyPolicy` fields will removed if previously set, defaulting to IPv4-only.
  ...
  - Alias A and AAAA records for the wildcard domain remain unchanged as they point to the same NLB hostname.
    However Alias AAAA record will not work anymore because IPv6 address for the load balancer will disappear.
